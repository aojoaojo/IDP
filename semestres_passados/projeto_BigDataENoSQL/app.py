...
def get_spell():
    ...
    for spell in result:
        print(spell['name'])
        # spellNome = result['name']
        # put_buttons([spellNome], onclick=lambda spellNome=spellNome: get_spell_especifica(spellNome))
...
def click_em_spell(spell):
    # Consultar spells no Mongodb    
    query = {}
    result = colecaoSpell.find(query)
        
    for document in result:
        if spell in document['name'].lower():
            put_markdown('### ' + document['name'])
            put_markdown('#### ' + document['description'])
